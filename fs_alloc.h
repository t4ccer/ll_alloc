...
unsigned char fs_alloced_chunks[(MAX_CHUNKS + (sizeof(size_t) * 8) - 1) / (sizeof(size_t) * 8)] = {0};
size_t fs_alloced_chunks_count = 0;

unsigned char is_allocated (size_t chunk_idx) {
  return fs_alloced_chunks[chunk_idx/ (sizeof(size_t) * 8)] & (1ul << (chunk_idx % (sizeof(size_t) * 8)));
}
...
void *fs_alloc () {
  ...
  for (int i = 0; i < MAX_CHUNKS; i++) {
    if (!is_allocated(i)) {
      fs_alloced_chunks[i / (sizeof(size_t) * 8)] |= (1ul << (i % (sizeof(size_t) * 8)));
